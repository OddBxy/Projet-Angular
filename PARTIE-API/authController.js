...
exports.getUser = async function(req, res) {
    try {
        db = await MongoClient.connect(url);
        let dbo = db.db("taches");
        let datas = await dbo.collection("utilisateur").find({login: req.body.login}).toArray();
...
exports.updateUser = async function(req, res) {
    try {
        db = await MongoClient.connect(url);
        let dbo = db.db("taches");
        await dbo.collection("utilisateur").updateOne({_id: mongodb.ObjectId(req.params.id)}, {$set: {login: req.body.login, password: req.body.password, listes: req.body.listes}});
        res.status(200).send();
    } catch (err) {
        console.log(err);
        res.status(500).json({ message: err })
